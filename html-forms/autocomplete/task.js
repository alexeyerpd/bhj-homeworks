class Autocomplete {
  ...
  hasMatches(option, text) {
    return option.textContent.toLowerCase().includes(text);
  }
  ...
  getMatches( text ) {
    ...
    return [...this.input.querySelectorAll("option")].reduce(
      (matches, option) => {
        if (this.hasMatches(option, text)) {
            matches.push(this.getOptionData(option));
        }
        return matches;
      },
      []
